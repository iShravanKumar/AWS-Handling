...
class CRUD(DB_CONNECTION):
    ...
    @staticmethod
    def restore(database_connection, database_name, table_name):
        try:
            database_cursor = database_connection.cursor()
            database_cursor.execute(f"DROP TABLE IF EXISTS {database_name}.{table_name}")
            restore_command = f'CREATE TABLE {database_name}.{table_name} ' \
                              f'SELECT * FROM {database_name}.{table_name}_backup'
            database_cursor.execute(restore_command)
            database_connection.commit()
            database_cursor.close()
            return True
        except mysql.connector.Error as error:
